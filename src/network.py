...
def Reformat_Graph(graph,edge_info):
	'''
	...
	edge_info -> {field: lambda function}
	'''
	...
	for idx0,node0 in enumerate(nodes):
		...
		for idx1 in adjacency_indices:
			...
			# Making the edge dictionary
			edge={}
			edge['vertices']=[idx0,idx1]
			edge['x']=[node0[0],node1[0]]
			edge['y']=[node0[1],node1[1]]

			for key,val in edge_info.items():

				edge[key]=val(graph._adj[node0][node1])
...
def Vertices_From_Graph(vertices,fields=[],x_field='x',y_field='y',edges_field='edges'):
	'''
	Converts NetworkX Graph into a vertices structure
	'''

	vertices={}

	for node_o in graph._node.keys():
